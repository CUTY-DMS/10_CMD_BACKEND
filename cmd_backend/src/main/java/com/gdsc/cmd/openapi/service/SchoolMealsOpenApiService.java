...
public class SchoolMealsOpenApiService {
    ...
    @Scheduled(cron = "0 0 0 * * *") //@Scheduled(cron = "초 분 시 일 월 요일")
    @SneakyThrows
    public void fetchMeals() throws Exception {

        LocalDate localDate = LocalDate.now(); // 현재 날짜를 객체로 받아옴. 나중에 .now 로 바꿔주기
        ...
        String jsonString = webClient
                .get()
                .uri("https://open.neis.go.kr/hub/mealServiceDietInfo?ATPT_OFCDC_SC_CODE=G10&SD_SCHUL_CODE=7430310&Key=21b0aabb876e40efa3e8f08a884c4544&MLSV_YMD="
                        +formattedDate+"&Type=json&pIndex=1&pSize=100")
                .retrieve()
            ...
            try {
                ...
                for (JsonNode row : rowNode) {
                    Meal meal = Meal.builder()
                            .MMEAL_SC_NM(row.path("MMEAL_SC_NM").asText())
